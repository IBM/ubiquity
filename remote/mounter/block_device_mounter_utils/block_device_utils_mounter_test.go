...
var _ = Describe("block_device_mounter_utils_test", func() {
	...
	Context(".DisconnectAll", func() {
		It("should fail if iscsi disconnect fail", func() {
			fakeBlockDeviceUtils.DisconnectReturnsOnCall(0, callErr)
			err = blockDeviceMounterUtils.DisconnectAll(volumeMountProperties)
			Expect(err).To(HaveOccurred())
			Expect(err).To(MatchError(callErr))
			Expect(fakeBlockDeviceUtils.DisconnectCallCount()).To(Equal(1))
			protocol, _ := fakeBlockDeviceUtils.DisconnectArgsForCall(0)
			Expect(protocol).To(Equal(block_device_utils.ISCSI))
		})
		It("should fail if scsi disconnect fail", func() {
			fakeBlockDeviceUtils.DisconnectReturnsOnCall(0, nil)
			fakeBlockDeviceUtils.DisconnectReturnsOnCall(1, callErr)
			err = blockDeviceMounterUtils.DisconnectAll(volumeMountProperties)
			Expect(err).To(HaveOccurred())
			Expect(err).To(MatchError(callErr))
			Expect(fakeBlockDeviceUtils.DisconnectCallCount()).To(Equal(2))
			protocol, _ := fakeBlockDeviceUtils.DisconnectArgsForCall(0)
			Expect(protocol).To(Equal(block_device_utils.ISCSI))
			protocol, _ = fakeBlockDeviceUtils.DisconnectArgsForCall(1)
			Expect(protocol).To(Equal(block_device_utils.SCSI))
		})
		It("should succeed to disconnect all", func() {
			fakeBlockDeviceUtils.DisconnectReturnsOnCall(0, nil)
			fakeBlockDeviceUtils.DisconnectReturnsOnCall(1, nil)
			err = blockDeviceMounterUtils.DisconnectAll(volumeMountProperties)
			Expect(err).NotTo(HaveOccurred())
			Expect(fakeBlockDeviceUtils.DisconnectCallCount()).To(Equal(2))
			protocol, _ := fakeBlockDeviceUtils.DisconnectArgsForCall(0)
