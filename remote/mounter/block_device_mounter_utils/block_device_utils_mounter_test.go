...
var _ = Describe("block_device_mounter_utils_test", func() {
	...
	Context(".CleanupAll", func() {
		It("should fail if iscsi cleanup fail", func() {
			fakeBlockDeviceUtils.CleanupDevicesReturnsOnCall(0, callErr)
			err = blockDeviceMounterUtils.CleanupAll(volumeMountProperties)
			Expect(err).To(HaveOccurred())
			Expect(err).To(MatchError(callErr))
			Expect(fakeBlockDeviceUtils.CleanupDevicesCallCount()).To(Equal(1))
			protocol, _ := fakeBlockDeviceUtils.CleanupDevicesArgsForCall(0)
			Expect(protocol).To(Equal(block_device_utils.ISCSI))
		})
		It("should fail if scsi cleanup fail", func() {
			fakeBlockDeviceUtils.CleanupDevicesReturnsOnCall(0, nil)
			fakeBlockDeviceUtils.CleanupDevicesReturnsOnCall(1, callErr)
			err = blockDeviceMounterUtils.CleanupAll(volumeMountProperties)
			Expect(err).To(HaveOccurred())
			Expect(err).To(MatchError(callErr))
			...
			protocol, _ := fakeBlockDeviceUtils.CleanupDevicesArgsForCall(0)
			Expect(protocol).To(Equal(block_device_utils.ISCSI))
			protocol, _ = fakeBlockDeviceUtils.CleanupDevicesArgsForCall(1)
			Expect(protocol).To(Equal(block_device_utils.SCSI))
		})
		It("should succeed to cleanup all", func() {
			fakeBlockDeviceUtils.CleanupDevicesReturnsOnCall(0, nil)
			fakeBlockDeviceUtils.CleanupDevicesReturnsOnCall(1, nil)
			err = blockDeviceMounterUtils.CleanupAll(volumeMountProperties)
			Expect(err).NotTo(HaveOccurred())
			Expect(fakeBlockDeviceUtils.CleanupDevicesCallCount()).To(Equal(2))
			protocol, _ := fakeBlockDeviceUtils.CleanupDevicesArgsForCall(0)
